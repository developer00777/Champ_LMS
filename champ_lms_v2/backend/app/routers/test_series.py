...
from app.services.test_attempts import (
    attempt_allowance,
    attempt_status,
    exhausted_message,
...
def _admin_view(t: TestSeries) -> dict:
    ...
    return {
        ...
        "is_ready": t.is_ready,
        # Audience, read-only here — it is set under Content access, which is
        # also where per-person grants live. Surfaced so an admin editing the
        # paper can see who it will reach without leaving the page.
        "audience_teams": t.audience_teams or [],
        "audience_departments": t.audience_departments or [],
        "target_roles": t.target_roles or [],
        "required_for_teams": t.required_for_teams or [],
...
async def _require_test_access(test: TestSeries, user: User) -> None:
    """
    Refuse the request unless this learner is in the test's audience.

    Runs on every learner-facing test endpoint, /submit included. Filtering the
    list alone would only hide the test: anyone holding the id could still POST
    a submission to a test they were never assigned.
    """
...
async def take_test(
    ...
    await _require_test_access(test, user)

    status = await attempt_status(test, user.id)
    if status["exhausted"]:
        raise HTTPException(status_code=403, detail=exhausted_message(status))
    used = status["used"]
...
async def submit_test(
    ...
    if not test.is_published:
        raise HTTPException(status_code=403, detail="This test is not published")
    # Same gate as /take. Checking only on the way in would let someone outside
    # the audience submit straight to this endpoint, and would let an attempt
    # opened before a revoke still score after it.
    await _require_test_access(test, user)
