...
<div class="page">
  ...
  {:else}
    ...
    <div class="form-card">
      ...
      <div class="audience">
        <span class="aud-lbl">Audience</span>
        <span class="aud-val">
          {#if test.is_restricted}
            {[
              test.audience_teams?.length ? `Teams: ${test.audience_teams.join(', ')}` : '',
              test.audience_departments?.length ? `Depts: ${test.audience_departments.join(', ')}` : '',
              test.target_roles?.length ? `Roles: ${test.target_roles.join(', ')}` : '',
              test.required_for_teams?.length ? `Required for: ${test.required_for_teams.join(', ')}` : '',
            ].filter(Boolean).join(' · ') || `Department: ${test.department}`}
          {:else}
            Everyone
          {/if}
        </span>
        <a class="aud-link" href="/admin/content-access">Change in Content access →</a>
      </div>
      <div class="row">
...
<style>
  ...
  .aud-lbl { color: var(--muted); font-size: 0.72rem; text-transform: uppercase; letter-spacing: 0.04em; }
  .aud-val { flex: 1; min-width: 12ch; }
  .aud-link { color: var(--accent); font-size: 0.78rem; text-decoration: none; white-space: nowrap; }
